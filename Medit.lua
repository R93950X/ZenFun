...
local function unnonlocalize(str)
    ...
    local line = "nonlocal "
    for t in m:gmatch "[%w%._]+" do
        local n = t:match "_G%.([%a_][%w_]*)"
        if not n then return m .. "=" end
        line = line .. (line == "nonlocal " and "" or ", ") .. n
    end
    return line .. " ="
    end)
end
...
local function load(_sPath)
    ...
    if fs.exists(_sPath) then
        ...
        while sLine do
            sLine = unnonlocalize(sLine) --Medit
...
local tMenuFuncs = {
    ...
    Run = function()
        ...
        local ok = save(sTempPath, function(file)
            file.write(runHandler:format(sTitle, 
            nonlocalize(table.concat(tLines, "\n")), -- Medit
             "@" .. fs.getName(sPath)))
        end)
        if ok then
